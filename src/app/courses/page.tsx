...
export default function CoursesPage() {
  ...
  const gradients = [
    { grad: "from-blue-500/10 to-indigo-500/10", text: "text-blue-600", lightBg: "bg-blue-500/15", bg: "bg-blue-500" },
    { grad: "from-purple-500/10 to-pink-500/10", text: "text-purple-600", lightBg: "bg-purple-500/15", bg: "bg-purple-500" },
    { grad: "from-emerald-500/10 to-teal-500/10", text: "text-emerald-600", lightBg: "bg-emerald-500/15", bg: "bg-emerald-500" },
    { grad: "from-orange-500/10 to-amber-500/10", text: "text-orange-600", lightBg: "bg-orange-500/15", bg: "bg-orange-500" },
  ];
      ...
      <div className="flex flex-col w-full py-8 text-foreground">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-14 pl-12 pr-12 bg-card border border-border rounded-[2rem] shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-foreground font-medium placeholder:text-muted-foreground"
            />
            {search && (
              <button
                type="button"
        ...
        {isLoading ? (
          <div className="w-full py-20 flex flex-col items-center justify-center">
            ...
            <p className="text-muted-foreground font-medium">강의 데이터를 불러오는 중입니다...</p>
          </div>
        ) : filteredCourses.length === 0 ? (
          <div className="w-full py-20 flex flex-col items-center justify-center bg-card border border-border rounded-[2rem] shadow-sm">
            {isInstructor ? (
              <GraduationCap className="w-16 h-16 text-muted-foreground/40 mb-4" />
            ) : (
            ...
            {isInstructor ? (
              ...
              <button
                onClick={handleEnrollByCode}
                className="h-12 px-6 bg-muted hover:bg-border text-foreground rounded-full font-bold transition-colors flex items-center"
              >
            ...
            {filteredCourses.map((course: any, idx: number) => {
              ...
              return (
                <Link
                  ...
                  className="group bg-card border border-border rounded-[2rem] overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col"
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <p className="text-sm text-muted-foreground mb-2 line-clamp-2 font-medium flex-1">
